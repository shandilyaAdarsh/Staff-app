...
class ProjectionRecoveryCoordinator {
  ...
  Future<void> executeRecovery({required String branchId}) async {
    ...
    _isRecovering = true;
    debugPrint('[Recovery] BEGIN authoritative projection recovery for branch: $branchId');
    ...
    try {
      ...
      debugPrint('[Recovery] STEP 2: Triggering server-side projection rebuild...');
      final response = await _dioClient.post('/runtime/projections/rebuild', data: {
        'branch_id': branchId,
      });

      if (response.data != null && response.data['success'] == true) {
        _rebuildGeneration = response.data['rebuild_generation'] ?? 0;
        debugPrint('[Recovery] Server rebuild success. Generation: $_rebuildGeneration');
      }

      // 3. Re-hydrate snapshot
      debugPrint('[Recovery] STEP 3: Re-fetching authoritative state snapshot...');
      ...
      final checksumResponse = await _dioClient.get('/runtime/projections/checksum');
      ...
      debugPrint('[Recovery] STEP 5: Resetting sync manager and resuming realtime subscriptions...');
      _syncManager.connectLocal();

      debugPrint('[Recovery] Authoritative projection recovery COMPLETED successfully.');
    } catch (e) {
...
final projectionRecoveryCoordinatorProvider = Provider<ProjectionRecoveryCoordinator>((ref) {
